...
void CustomWindowPrivate::init(
	...
	btn_stayOnTop = new CustomButton(
		w_topBorder, true,
		"Stay on top",
		px_stayOnTopButton,
	...
	QObject::connect(
		btn_stayOnTop, SIGNAL(clicked(bool)),
	...
	QObject::connect(
		btn_minimize, SIGNAL(clicked()),
		q, SLOT(showMinimized())
	);

	ll->addWidget(btn_minimize);

	btn_close = new CustomButton(
		w_topBorder, false,
		"Close",
		px_closeButton,
		q->tr("Close")
	...
	QObject::connect(
		btn_close, SIGNAL(clicked()),
		q, SLOT(close())
	);

	ll->addWidget(btn_close);

	int maxButtonHeight = qMax(
		btn_stayOnTop->height(),
