...
void MainButton::mouseReleaseEvent ( QMouseEvent * event )
 ...
 void MainButton::enterEvent ( QEvent * event )
  {Q_UNUSED(event);
   ...
   qDebug("mouseOnWidget");
  };
 void MainButton::leaveEvent ( QEvent * event )
  {Q_UNUSED(event);
  if(mouseOver){mouseOver=false;repaint();qDebug("mouseOffWidget");};
  };
	...
	: QWidget ( parent, fl ), Ui::RoboPult()
{
	...
	buttRad->setText(trUtf8(" "));
        if(!buttRad->loadIcon(":/icons/radiation.png"))qDebug("Image not loaded!");
	TempB->hide();
...
void RoboPult::noLink(){
	link=false;
        greenLight->setLink(link);greenLight->repaint();qDebug("NoLINK");};
void RoboPult::LinkOK(){link=true;greenLight->setLink(link);greenLight->repaint();};

void RoboPult::Up()
  {
  if(askStena->isChecked () ){emit hasUpWall(); askStena->setChecked(false);qDebug("Up Stena");;return;};
