...
Size TableViewerPrivate::getSize(int dimension) const
{
    ...
    if (module) {
        symbol_table * table = module->Values();
        int a = 0, b = 0;
        if (dimension==1 && table->symb_table[variableId].razm==1) {
            a = table->symb_table[variableId].size0_start;
            b = table->symb_table[variableId].size0_end;
        }
        else if (dimension==1 && table->symb_table[variableId].razm==2) {
            a = table->symb_table[variableId].size0_start;
            b = table->symb_table[variableId].size0_end;
        }
        else if (dimension==1 && table->symb_table[variableId].razm==3) {
...
QString TableViewerPrivate::getValue(int dim, int slice, int row, int col) const
{
    ...
    if (module) {
        symbol_table * table = module->Values();
        if (dim==1) {
            value = table->symb_table[variableId].value.getVariantMassValue2aaa(col,0,0);
        }
        if (dim==2) {
            value = table->symb_table[variableId].value.getVariantMassValue2aaa(col,col,0);
        }
        if (dim==3) {
            value = table->symb_table[variableId].value.getVariantMassValue2aaa(col,row,slice);
        }
...
TableViewer::TableViewer(KumModules *modules, int modId, int varId, QWidget *parent)
    : QTabWidget(parent)
{
