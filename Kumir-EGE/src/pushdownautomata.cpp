...
PushdownAutomata::PushdownAutomata ( QObject *parent )
	: AutomataData ( parent )
{
    ...
    initFilePath = QDir::cleanPath(QCoreApplication::applicationDirPath()+"/../share/kumir/pdscripts/pd_init.js");
...
QPoint PushdownAutomata::loadRules(QIODevice *source)
{
	...
	foreach ( QString line, lines ) {
		...
		if ( terminal.isEmpty() ) {
			...
			Kum::InstructionType allLines[] = { Kum::Empty, Kum::VarDecl,Kum::AlgDecl, Kum::AlgBegin, Kum::AlgEnd,
												Kum::LoopBegin, Kum::LoopEnd, Kum::LoopEndCond, Kum::If, Kum::Then, Kum::Else,
												Kum::Fin, Kum::Switch, Kum::Case, Kum::Exit, Kum::Assign,
												Kum::Assert, Kum::Pre, Kum::Post, Kum::Input, Kum::Output,
												Kum::FInput, Kum::FOutput, Kum::Doc, Kum::AlgCall, Kum::ModBegin,
												Kum::ModEnd, Kum::UseMod, Kum::Undefined};
			...
			do {
				...
				j++;
			} while ( allLines[j] != Kum::Undefined );
		}
		else {
...
uint PushdownAutomata::process()
{
	...
	do {
		...
		if ( m_matrix.contains(key) ) {
			Rules rulesList = m_matrix[key];
			if ( ALLOW_QDEBUG ) {
				qDebug() << "ALL RULES: " << rulesList.size();
...
void AutomataData::init(const QList<ProgaText> &source, const QString &fileName)
{
	m_text.clear();
	ProgaText pt;
	pt.Text = "begin";
	pt.Base_type = Kum::Undefined;
	m_text << pt;
	m_text << source;
...
void PushdownAutomata::init(const QList<ProgaText> &source, const QString &fileName)
{
	AutomataData::init(source, fileName);
...
bool PushdownAutomata::postProcess()
{
	if ( ALLOW_QDEBUG ) {
		qDebug() << "SCRIPTS :";
		QString a;
...
QString PushdownAutomata::terminalByCode(Kum::InstructionType code)
{
	switch ( code ) {
	case Kum::VarDecl:
...
bool PushdownAutomata::isCorrectTerminal(const QString &terminal)
{
	if (
			...
			||
			//			 terminal == QString::fromUtf8("лог")
			//			 ||
			//			 terminal == QString::fromUtf8("вещ")
			//			 ||
			//			 terminal == QString::fromUtf8("сим")
			//			 ||
			//			 terminal == QString::fromUtf8("лит")
			//			 ||
...
protected:
	inline void run() {
		m_result = m_automata->process();
		if ( m_result == 0 ) {
			m_scriptResult = m_automata->postProcess();
		}
		else  {
