...
int set_const_value(int perem_id,const QString &val,symbol_table *symbols)
{
    ...
    // NEW 28.08.2006
    if (symbols->getTypeByID(perem_id)==boolean)
    {
        bool value = text_analiz::createConstValue(val,boolean).toBool();
                                        ...
                                        QString *root)
{
    ...
    return parceExpression(viraj,table,symbols,algorithm,err,parsedViraj,&structType,err_pos, extraSubExpressions, isLeft,root);
}
        ...
        )
{
        ...
        case Sim_funct:
            {
                ...
                if ( not_flag )
                {
                    if ( modules->module(module_id)->Functions()->typeById(func_id)!=boolean )
                    {
                        QPoint pnt = translateErrorPosition(not_pos,not_pos+1,P,L);
                        *err_pos = ErrorPosition(pnt.x(),pnt.y());
                        *err = GNVR_NOT_LOG;
        ...
        case Sim_perem:
            {
                ...
                if ( func_id==-1 )
                {
                    ...
                    //	a) work NOT (check if boolean later)
    ...
    *err=0;
    ...
    for ( int i=0; i<exprList.count(); i++ )
    {
        ...
        pt = G ( exprList[i],&local_error,table,symbols,exprTypeList,exprStructList,exprList,&st,&local_err_pos,&local_err_len,include_bounds_in_error, &parsedNodes, root==NULL? NULL : &expr);
...
int text_analiz::parceAlgorhitmCall(proga &pv,function_table *table,symbol_table *symbols,int str,QString *xml)
{
    ...
    if(skobk_o==-1)
    {
        ...
        if (xml!=NULL) {
            *xml = "<call module='"+modules->module(module_id)->name+"' algorhitm='"+modules->module(module_id)->Functions()->nameById(func_id)+"'/>\n";
        }
        // NEW V.Y. 21.08.2006
        if ( modules->module(module_id)->Functions()->argCountById(func_id) > 0 )
        {
...
{
    if((inType==integer)||(inType==charect)||(inType==real)||(inType==boolean))
    {
        return inType;
    };
    if((inType==kumString))
...
int text_analiz::parceAssignment(proga &pv,function_table *table,symbol_table *symbols,int str, QString *root)
{
...
int text_analiz::parceVariablesDeclaration(proga &pv,int str,QString alg,symbol_table *symbols,function_table *table, QString *xml)
{
    ...
    int local_err = parceVariables(stroka,ispId,alg,arg,str,symbols,table,local_err_pos,ids,pv.VirajList,pv.extraSubExpressions,xmlList,false,extraProgram);
    Err_start = local_err_pos.start;
    Err_length = local_err_pos.len;
    int counter = 0;
    pv.line = "";
...
* @return Код ошибки
 */
int text_analiz::parce_if(proga &pv,symbol_table *symbols,function_table *functions,int str,QString *xml)
{
...
* @return Код ошибки.
 */
int text_analiz::parseLoopBegin(proga &pv,symbol_table *symbols,function_table *functions,int str,QString *xml)
{
    QString instr = pv.line;
    int iz, iz1;
    int circle_type=0;
    ...
    if (circle_type==FOR_CIRCLE)
    {
        ...
        if (xml!=NULL)
            *xml += "<to>\n";
        intType = parceExpression2(viraj,functions,symbols,functions->nameByPos(str),&err,&parsedViraj,&local_err_pos,pv.extraSubExpressions,false,xml);
        if (xml!=NULL)
            *xml += "</to>\n";
        if(err>0)
        {
    ...
    if (circle_type==RAZ_CIRCLE)
    {
        ...
        if (xml!=NULL)
            *xml += "<times>\n";
        PeremType intType=parceExpression2(viraj,functions,symbols,functions->nameByPos(str),&err,&parsedViraj,&local_err_pos,pv.extraSubExpressions,false,xml);
        if (xml!=NULL)
            *xml += "</times>\n";
        if(err>0)
        {
    ...
    if (circle_type==WHILE_CIRCLE)
    {
        ...
        PeremType intType=parceExpression2(viraj,
                                           ...
                                           &local_err_pos,pv.extraSubExpressions,false,xml);
        if (xml!=NULL)
            *xml += "</while>\n";
        if(err>0)
        {
...
{
    QString prisv = pv.line;
    int f_id=modules->idByName("Files");
...
int text_analiz::parce_dano(proga &pv,function_table *functions,symbol_table *symbols,int str,QString *xml)
{
    QString instr = pv.line;
    ...
    PeremType     intType=parceExpression2(uslovie,functions,symbols,functions->nameByPos(str),&err,&parsedViraj,&local_err_pos,pv.extraSubExpressions,false,xml);
    if (xml!=NULL) {
        *xml += "</input_assert>\n";
    }
...
int text_analiz::parce_nado(proga &pv,function_table *functions,symbol_table *symbols,int str,QString *xml)
{
    QString instr = pv.line;
    pv.VirajList.clear();
    if(functions->nameByPos(str)=="global")
...
int text_analiz::parceInput(proga &pv,function_table *functions,symbol_table *symbols,int str,QString *xml)
        //16-08-Mordol
{
    QString instr = pv.line;
    ...
    for ( int t=0; t<terms.count(); t++ )
    {
        ...
        if(op_sk<1)
        {
            ...
            PeremType perem_type = parceExpression(uslovie,functions,symbols,functions->nameByPos(str),&err,&parsedViraj,&st,&local_err_pos,pv.extraSubExpressions,false,xml);
        ...
        if(op_sk>=1)
        {
            ...
            if(symbols->getTypeByID(p_id)==integer
               ||symbols->getTypeByID(p_id)==boolean
...
int text_analiz::parce_fvvod(proga &pv,function_table *functions,symbol_table *symbols,int str,QString *xml)
        //16-08-Mordol
{
    QString instr = pv.line;
    QList<int> mass_arg_starts;
...
* @return Код ошибки
 */
int text_analiz::parce_pri(proga &pv,function_table *functions,symbol_table *symbols,int str,QString *xml)
{
    QString instr = pv.line;
    //if(functions->nameByPos(str)=="global")return SYNTAX_ERR;
    QString uslovie="";
    ...
    PeremType       intType=parceExpression2(uslovie,functions,symbols,functions->nameByPos(str),&err,&parsedViraj,&local_err_pos,pv.extraSubExpressions,false,&xmlCondition);
    if (xml!=NULL) {
...
int text_analiz::parce_utv(proga &pv,function_table *functions,symbol_table *symbols,int str,QString *xml)
{
    QString instr = pv.line;
...
int text_analiz::parce_kc_pri(proga &pv,function_table *functions,symbol_table *symbols,int str,QString *xml)
{
    QString instr = pv.line;
    //    int to_pos=instr.indexOf(QChar(KS_TO));
...

{
    ...
    if ( l==-1 )
    {
        ...
        int local_err_start;
        int local_err_len;
        PeremType p = getObjType(s,&local_err,table,symbols,type_list,struct_list,expr_list,strNew,local_err_start,local_err_len,include_bounds_in_error,parsedNodes,root);
        if ( local_err > 0 )
        {
            *err = local_err;
    ...
    else
    {
        ...
        {
            leftType = G(H,&local_err,table,symbols,type_list,struct_list,expr_list,&leftStruct,&local_err_start,&local_err_len, include_bounds_in_error,parsedNodes,&leftXml);
        ...
        {
            rightType = G(T,&local_err,table,symbols,type_list,struct_list,expr_list,&rightStruct,&local_err_start,&local_err_len, include_bounds_in_error,parsedNodes,&rightXml);
    ...
    if ( work_block[0]==QChar ( TYPE_SKOBKA ) )
    {
        ...
        if (root!=NULL) {
            ...
            if (symbols->symb_table[p_id].is_FunctionReturnValue)
                ...
                id = p_id;
            Q_CHECK_PTR(parsedNodes);
            if (*StrNew==string_elem) {
                *root += "<string_element id='"+QString::number(id)+"'>\n";
...
// 										if ( id <= symbols->count )
// 										{
// 											if ( symbols->symb_table[id].is_const && symbols->symb_table[id].type == boolean )
// 											{
// 												result++;
// 											}
// 										}
...
int text_analiz::parce_kc(QString instr)
{
    if ( instr.trimmed().length() > 1 )
    {
        Err_start = 1;
...
int text_analiz::parce_isp(QString instr,function_table *table,symbol_table *symbols,int str, int *err_start,int *err_len,QString file,QString *returnName)
{
    Err_start = 1;
    Err_length = instr.length()-1;
    return 65534;
...
{ //TODO переделать по людски
    ...
    *err_end = instr.size();
            return 65534;
    Q_UNUSED(str);
    int strtype=KumTools::instance()->getStringType ( instr );
    *fileName="";
    if((strtype!=Kum::UseMod)&&(strtype!=Kum::Empty))return -10;
    if(instr[2]!='\"')
    {
    ...
    for ( int curPos=0, nameStart=0, arrayStart=0, arrayBoundStart=0, typePos=0; curPos<source.length(); curPos++ )
    {
        ...
        if ( par == name )
        {
            ...
            if ( source[curPos] == '[' )
            {
                ...
                {
                    if ( (cType == integer) || (cType == real) || (cType == boolean) || (cType == kumString) || (cType == charect) )
                    {
                        err_pos.start = curPos;
            ...
            else if ( (source[curPos] == ',') || (source[curPos] == ';') || ( source[curPos]=='=' ) )
            {
                ...
                if (
                        ! ( (cType == integer) || (cType == real) || (cType == boolean) || (cType == kumString) || (cType == charect) )
                        &&
                        ! massDeclared
                        )
                {
        ...
        else if ( par == lb )
        {
            ...
            else if ( source[curPos] == ':' )
            {
                ...
                QString leftXml;
                PeremType leftType = parceExpression2(cBound.trimmed(),functions,symbols,algName,&locErr,&el,&locErrPos,extraSubExpressions,false,&leftXml);
                if ( locErr > 0 )
                {
        ...
        else if ( par == rb )
        {
            ...
            else if ( (source[curPos] == ',') && (deep1 == 0) && (deep2 == 0) )
            {
                ...
                ErrorPosition locErrPos;
                QString rightXml;
                PeremType rightType = parceExpression2(cBound,functions,symbols,algName,&locErr,&el,&locErrPos,extraSubExpressions,false,&rightXml);
                if ( locErr > 0 )
                {
            ...
            if ( deepV==0 && ( source[curPos]==',' || source[curPos]==';' )) {
                ...
                if ( cType==integer || cType==real || cType==boolean || cType==charect || cType==kumString ) {
...
int text_analiz::parceConstant(const QString &constant, const PeremType pt, int& maxDim, QString &value, int &errPos, int &errLen)
{
    ...
    if (pt==integer || pt==real || pt==boolean || pt==charect || pt==kumString ) {
...
int text_analiz::parceAlgorhitmHeader(proga &pv, int str, symbol_table *symbols, function_table *functions)
{
    ...
    if ( returnType != none )
    {
        ...
        if ( local_error > 0 )
        {
            ...
            // то это внутренняя ошибка КУМИРа
            return INT_TA_11;
        }
        symbols->new_symbol(symbols->count);
    ...
    if ( argLine.startsWith("(") && argLine.endsWith(")") )
    {
        ...
        for ( int i=0; i<groups.count(); i++ )
        {
            ...
            ErrorPosition locErrPos;
            QList<proga> extraProgram;
            locErr = parceVariables(groups[i],isp_id,name,mods[i],str,symbols,functions,locErrPos,ids,pv.VirajList,pv.extraSubExpressions,xmlList,true,extraProgram);
            if ( locErr > 0 )
            {
                                     ...
                                     RealLine realLine)
{
    ...
    if (callReturnType!=none) {
        ...
        else if (callReturnType==boolean)
    ...
    for( int i=0; i<input.count(); i++ ) {
        ...
        // вывод "Введите ${имя_переменной} :"
#ifndef EGE_MODE
        inp = input[i];
        declNo = inpDeclNo[i];
        cpv.str_type = Kum::Output;
        cpv.VirajList.clear();
        cpv.VirajTypes.clear();

        cpv.line = QString(QChar(KS_VIVOD))+" \""+screenString(QString::fromUtf8("Введите ")+inp+": ")+"\"";
        error = parceOutput(cpv, functions, symbols, -1, &xml);
        K_ASSERT ( error==0 );
        m_hiddenAlgorhitm.append(cpv);
#endif
        ...
        cpv.str_type = Kum::VarDecl;
        cpv.line = declarations[declNo];
        cpv.VirajList.clear();
        cpv.VirajTypes.clear();
        error = parceVariablesDeclaration(cpv, -1, "", symbols, functions, &xml);
        K_ASSERT ( error==0 );
        m_hiddenAlgorhitm.append(cpv);
        ...
        cpv.str_type = Kum::Input;
        cpv.VirajList.clear();
        cpv.VirajTypes.clear();
        cpv.line = QString(QChar(KS_VVOD))+" "+inp;
        error = parceInput(cpv, functions, symbols, -1, &xml);
        K_ASSERT ( error==0 );
        m_hiddenAlgorhitm.append(cpv);
    }
    ...
    // Формируем строки объявления переменных рез-параметров
    ...
    if ( callReturnType==none ) {
        cpv.str_type = Kum::AlgCall;
        cpv.VirajList.clear();
        cpv.VirajTypes.clear();
        cpv.line = callName+"("+arguments.join(",")+")";
        error = parceAlgorhitmCall(cpv,functions,symbols,-1,&xml);
        K_ASSERT ( error==0 );
        m_hiddenAlgorhitm.append(cpv);
    }
    else {
        ...
        cpv.VirajTypes.clear();
#ifndef EGE_MODE
        cpv.line = QString(QChar(KS_VIVOD))+" \""+screenString(QString::fromUtf8("Значение функции = "))+"\","+callName+"("+arguments.join(",")+"),"+QString(QChar(KS_NS));
#else
        cpv.line = QString(QChar(KS_VIVOD))+" "+callName+"("+arguments.join(",")+")";
#endif
...
int text_analiz::parce_doc(QString instr,function_table *functions,symbol_table *symbols,int str,QString *xml)
{
    Q_UNUSED(xml);
    Q_UNUSED(symbols);
...
int text_analiz::parceFuncList(int module_id,QStringList funcList)
{
    ...
    for(int i=0;i<funcList.count();i++)
    {
        ...
        Kum::InstructionType sttype=KumTools::instance()->getStringType(normaString);
        pv.str_type=sttype;
        if(err!=0){return -1;};
        if(pv.str_type==Kum::AlgDecl)
...
void text_analiz::analizeModule(int module_id)
{
    ...
    for ( int i=0; i<module->Proga_Value()->count(); i++ ) {
        ...
        case Kum::If: {
                ...
                error = parce_if ( pv, symbols, functions, i, &xml);
                break;
            }
        ...
        case Kum::Case: {
                ...
                error = parce_pri(pv,functions,symbols,i,&xml);
                break;
            }
        ...
        case Kum::FOutput: {
                error = parce_fvivod(pv,functions,symbols,i,&xml);
                break;
            }

        case Kum::Assert: {
                error = parce_utv(pv,functions,symbols,i,&xml);
                break;
            }

        case Kum::LoopEndCond: {
                error = parce_kc_pri(pv,functions,symbols,i,&xml);
                m_denyVariableDeclaration --;
                break;
            }

        case Kum::Pre: {
                error = parce_dano(pv,functions,symbols,i,&xml);
                break;
            }

        case Kum::Post: {
                error = parce_nado(pv,functions,symbols,i,&xml);
                break;
            }
        ...
        case Kum::FInput: {
                error = parce_fvvod(pv,functions,symbols,i,&xml);
                break;
            }

        case Kum::Doc: {
                error = parce_doc(pv.line,functions,symbols,i,&xml);
                break;
            }
    ...
    if ( !m_hiddenAlgorhitm.isEmpty() ) {
        ...
        int firstId = module->Functions()->idByPos(m_firstAlgorhitmIP);
        module->Functions()->setInitializationIP(firstId, hiddenIP);
        module->Functions()->setHiddenAlgorhitmPos(hiddenIP,
                                                   module->progaValueCount());
    }

    module->setInitialIP(0);
    module->syncTables();
