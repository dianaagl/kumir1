...
int PWRulesNormalizer::process(const QStringList &files, QIODevice *out, QList<int> priors)
{
  ...
  //qDebug() << files;
	...
	if (files.isEmpty())
	{
	  //		qWarning()<<QString::fromUtf8("Нет .rules-файлов");
		return 2;
	}
...
// 		qDebug() << "pravila1:: " << s;
	...
	for(int i=0; i<pravila1.size(); ++i)
	{
		...
		if (d!=d1)
		{
//			qWarning()<<QString::fromUtf8("Два двоеточия в строке: ")<<str<<endl;
			return 1;
		}
...
// 		if (e!=e1)
// 		{
// 			qWarning()<<QString::fromUtf8("Два приоритета в строке: ")<<str<<endl;
// 			return 1;
// 		}
		if (e==-1)
		{
//			qWarning()<<QString::fromUtf8("Не задан приоритет строки: ")<<str<<endl;
			return 1;
		}
		if (d!=-1)
		{
		...
		{
//			qWarning()<<QString::fromUtf8("Ошибка задания приоритета в строке: ")<<str<<endl;
			return 1;
		}
...
// 		qDebug() << "plavila: " << pravila.last();
		...
		{
//			qWarning()<<QString::fromUtf8("Ошибка в левой части: ")<<pravila.last().join(" ")<<endl;
			return 1;
		}
	}
	...
	foreach(QString s, nagruzki1)
	{
		...
		{
//			qWarning() << QString::fromUtf8("Не совпадает число терминалов и нагрузок: ") << pravila[ind].join(" ") << " : " << s;
			return 1;
		}
		nagruzki << rez;
		++ind;
...
// 		qDebug() << i << ": " << pravila[i];
	...
	for(bool flag = true ; flag ;)
	{
		...
		{
//			qWarning() << QString::fromUtf8("Данная грамматика является леворекурсивной\n");
			return 1;
		}
		else
		{
	...
	for(c=0; c<pravila.size(); ++c)
	{
		...
		for(int j=c+1; j<pravila.size(); )
		{
			...
			if (pravilo==pravilo1)
			{
				...
				{
//					qWarning() << "Dva pravila s raznoj nagruzkoj ili raznymi prioritetami:";
//					qWarning() << pravilo << " : " << nagruzka << " [" << priority << "]";
//					qWarning() << pravilo1 << " : " << nagruzka1 << " [" << priority1 << "]";
					++j;
				}
			}
			else
