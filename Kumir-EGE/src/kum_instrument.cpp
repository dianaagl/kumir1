...
int KumStandartFunctions::getFunctionList(function_table* Functions,symbol_table* Symbols)
{
    ...
    Functions->setArgType(Functions->size()-1, 0, kumString);
    Functions->setArgType(Functions->size()-1, 1, boolean);
...
{ 
	...
	*err=0;
    ...
    if (name==trUtf8("модуль"))
    {
        ...
        if (name==trUtf8("минимум"))
        {
        int x = arguments[0].asInt();
        int y = arguments[1].asInt();
        int perem_id=symbols->inTable(trUtf8("минимум"), "global");
        if ( x > y )
        ...
        if (name==trUtf8("максимум"))
        {
        int x = arguments[0].asInt();
        int y = arguments[1].asInt();
        int perem_id=symbols->inTable(trUtf8("максимум"), "global");
        if ( x < y )
            symbols->symb_table[perem_id].value->setIntegerValue(y);
        else
            symbols->symb_table[perem_id].value->setIntegerValue(x);
        ...
        if (name==trUtf8("случайное число")) //TODO WINDOWS
        {
        // #ifndef WIN32
        int x = arguments[0].asInt();
		...
		int perem_id=symbols->inTable(trUtf8("случайное число"), "global");
		symbols->symb_table[perem_id].value->setIntegerValue(result);
		// #else
		//     int x = *(int*)virajList[0];
		//     long int rndValue = rndGenerator->random();
		//     double stdValue = (double)rndValue/(rndGenerator->maxValue()); // as R(0;1)
		//     int result = (int)((double)x*stdValue);
		//     int perem_id=symbols->inTable(trUtf8("случайное число"), "global");
		//     symbols->symb_table[perem_id].value->setIntegerValue(result);
		// #endif
	...
	if (name=="sin")
	{
		...
		symbols->symb_table[perem_id].value->setFloatValue(sinf(x));
	}else
		...
		symbols->symb_table[perem_id].value->setFloatValue(cosf(x));
	}else
		...
		symbols->symb_table[perem_id].value->setFloatValue(atanf(x));
	}else
