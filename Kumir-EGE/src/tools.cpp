 ...
 * @return Тип
 */
Kum::InstructionType KumTools::getStringType ( QString string )
{
    uint first_b=string[0].unicode();
    if ( string.startsWith ( trUtf8 ( "|" ) ) )
        return Kum::Comment;
    else if (first_b==KS_INCLUDE)
        return Kum::Include;
    else if ( first_b==KS_ALG )
        return Kum::AlgDecl;
    else if ( first_b==KS_NACH )
        return Kum::AlgBegin;
    else if ( first_b==KS_KON )
        return Kum::AlgEnd;
    else if ( first_b==KS_CEL )
        return Kum::VarDecl;
    else if ( first_b==KS_CEL_TAB )
        return Kum::VarDecl;
    else if ( first_b==KS_LIT )
        return Kum::VarDecl;
    else if ( first_b==KS_LIT_TAB )
        return Kum::VarDecl;
    else if ( first_b==KS_SIM )
        return Kum::VarDecl;
    else if ( first_b==KS_SIM_TAB )
        return Kum::VarDecl;
    else if ( first_b==KS_LOG_TAB )
        return Kum::VarDecl;
    else if ( first_b==KS_LOG )
        return Kum::VarDecl;
    else if ( first_b==KS_VES )
        return Kum::VarDecl;
    else if ( first_b==KS_VES_TAB )
        return Kum::VarDecl;
    else if ( first_b==KS_NC )
        return Kum::LoopBegin;
    else if ( first_b==KS_KC )
        return Kum::LoopEnd;
    else if ( first_b==KS_ESLI )
        return Kum::If;
    else if ( first_b==KS_VIBOR )
        return Kum::Switch;
    else if ( first_b==KS_PRI )
        return Kum::Case;
    else if ( first_b==KS_TO )
        return Kum::Then;
    else if ( first_b==KS_VSE )
        return Kum::Fin;
    else if ( first_b==KS_VIVOD )
        return Kum::Output;
    else if ( first_b==KS_FVIVOD )
        return Kum::FOutput;
    else if ( first_b==KS_FVVOD )
        return Kum::FInput;
    else if ( first_b==KS_INACHE )
        return Kum::Else;
    else if ( first_b==KS_UTV )
        return Kum::Assert;
    else if ( first_b==KS_DANO )
        return Kum::Pre;
    else if ( first_b==KS_NADO )
        return Kum::Post;
    else if ( first_b==KS_DOC )
        return Kum::Doc;
    else if ( first_b==KS_VVOD )
        return Kum::Input;
    else if ( first_b==KS_KC_PRI )
        return Kum::LoopEndCond;
    else if ( first_b==KS_EXIT )
        return Kum::Exit;
    else if ( first_b==KS_ISP )
        return Kum::ModBegin;
    else if ( first_b==KS_ENDISP )
        return Kum::ModEnd;
    else if ( first_b==KS_USE )
        return Kum::UseMod;
    else if ( string.count ( QChar ( KS_PRISV ) ) >=1 )
        return Kum::Assign;
    else if ( string.isEmpty() || string.isNull() )
        return Kum::Empty;
    else
        return Kum::AlgCall;
}
...
int findMinSkobka ( QString stroka )
{
    ...
    if ( test<0 ) return -1;
    int skobk_count=0,skobk_pos=0,min_skobk=-1;
    skobk_count=0;
...
int KumTools::splitUserStr ( QString stroka,QList<ProgaText>* result,int str )
{
    ...
    Kum::InstructionType RealLineType=getStringType ( norma_string );
    QStringList List;
    List.clear();
    switch ( RealLineType )
    {
 ...
 * @return
 */
int KumTools::splitFStringList ( QStringList FSpisok,QList<ProgaText>* result,int realLine,int Last_F_ID,int  offset,QString file, const QList<QList<int> > &PPs, const QList<QList<int> > &LLs )
{
...
int KumTools::splitUserFile ( QFile &file, QList< QList<ProgaText> > *result )
{
    ...
    if ( file.open(QIODevice::ReadOnly|QIODevice::Text) ) {
        ...
        for ( int i=0; i<all.count(); i++ ) {
            if ( getStringType(all[i].Text) == Kum::ModBegin) {
                if ( inIsp ) {
            ...
            else if ( getStringType(all[i].Text) == Kum::ModEnd) {
                inIsp = false;
                if ( inInnerIsp ) {
                    all[i].Error = TOS_ISP_IN_ISP;
                    all[i].errstart = 0;
...
{
    QPoint result;
    int st = KumTools::getStringType(pt.Text);
    if ( st == Kum::AlgBegin) // 13, нач
    {
        result = QPoint(0,1);
    }
    else if ( st == Kum::AlgEnd) // 14, кон
    {
        result = QPoint(-1,0);
    }
    else if ( st == Kum::LoopBegin) // 15, нц
    {
        result = QPoint(0,1);
    }
    else if ( st == Kum::LoopEnd ) // 16, кц
    {
        result = QPoint(-1,0);
    }
    else if ( st == Kum::LoopEndCond ) // 33, кц_при
    {
        result = QPoint(-1,0);
    }
    else if ( st == Kum::Else ) // 17, иначе
    {
        result = QPoint(-1,1);
    }
    else if ( st == Kum::If ) // 20, если
    {
        result = QPoint(0,2);
    }
    else if ( st == Kum::Then ) // 21, то
    {
        result = QPoint(-1,1);
    }
    else if ( st == Kum::Fin ) // 22, все
    {
        result = QPoint(-2,0);
    }
    else if ( st == Kum::Switch ) // 23, выбор
    {
        result = QPoint(0,2);
    }
    else if ( st == Kum::Case ) // 24, при
    {
        result = QPoint(-1,1);
    }
    else if ( st == Kum::Pre ) // 30, дано
    {
        result = QPoint(1,-1);
    }
    else if ( st == Kum::Post ) // 31, надо
    {
        result = QPoint(1,-1);
    }
    else if ( st == Kum::ModBegin ) // 34, исп
    {
        result = QPoint(0,1);
    }
    else if ( st == Kum::ModEnd ) // 35, кон_исп
    {
        result = QPoint(-1,0);
    }
    else if ( st == Kum::Doc ) // 98, #
    {
        result = QPoint(1,-1);
    }
    else
...
//	bool tQuoted = lexem2.startsWith("<") && lexem2.endsWith(">");
	...
	if (link.toLower().startsWith("http://")) {
		...
		if (fi.isAbsolute()) {
			...
			const QString rootDir = QDir::currentPath();
	...
	if ( ok )
		...
		foreach ( InputTerm word, words )
		{
			if ( !word.valid && word.start>-1) {
				msg = QCoreApplication::translate("KumIOArea","INPUT ");
				QStringList m;
				foreach ( InputTerm word, words ) {
					QString t;
					if ( word.pType == integer )
						t = QCoreApplication::translate("KumIOArea","int");
					else if ( word.pType == real )
						t = QCoreApplication::translate("KumIOArea","float");
					else if ( word.pType == charect )
						t = QCoreApplication::translate("KumIOArea","char");
					else if ( word.pType == kumString )
						t = QCoreApplication::translate("KumIOArea","string");
					else if ( word.pType == boolean )
						t = QCoreApplication::translate("KumIOArea","bool");
					if ( word.valid )
						m << t;
				...
				for ( int i=words.count(); i<types.count() ; i++ ) {
					if ( types[i]==integer ) m << QCoreApplication::translate("KumIOArea","int");
					if ( types[i]==real ) m << QCoreApplication::translate("KumIOArea","float");
					if ( types[i]==charect ) m << QCoreApplication::translate("KumIOArea","char");
					if ( types[i]==kumString ) m << QCoreApplication::translate("KumIOArea","string");
					if ( types[i]==boolean ) m << QCoreApplication::translate("KumIOArea","bool");
				}
				msg += m.join(", ")+".";
				if ( word.text.length()>0 )
				{
					if ( word.error == notInteger )
						msg += QCoreApplication::translate("KumIOArea","It is not integer.");
					else if ( word.error == notReal )
						msg += QCoreApplication::translate("KumIOArea","It is not real.");
					else if ( word.error == notBool )
						msg += QCoreApplication::translate("KumIOArea","It is not boolean.");
					else if ( word.error == unpairedQuote )
						msg += QCoreApplication::translate("KumIOArea","Unpaired quote.");
					else if ( word.error == longCharect )
						msg += QCoreApplication::translate("KumIOArea","More then one character.");
					else if ( word.error == quoteMisplace )
						msg += QCoreApplication::translate("KumIOArea","Quotes misplaced.");
					else if ( word.error == differentQuotes )
						msg += QCoreApplication::translate("KumIOArea","Different quotes.");
				}
				else {
					if ( word.pType==integer )
						msg += QCoreApplication::translate("KumIOArea","Integer not entered.");
					if ( word.pType==real )
						msg += QCoreApplication::translate("KumIOArea","Real not entered.");
					if ( word.pType==boolean )
						msg += QCoreApplication::translate("KumIOArea","Boolean not entered.");
					if ( word.pType==charect )
						msg += QCoreApplication::translate("KumIOArea","Charect not entered.");
				}
				return ok;
			}
			else if ( !word.valid ) {
		...
		if ( words.count() < types.count() ) {
			...
			if ( pType==charect )
				msg += QCoreApplication::translate("KumIOArea","Charect not entered.");
			if ( pType==kumString )
				msg += QCoreApplication::translate("KumIOArea","String not entered.");
		}
		if ( inputGarbageStart > -1 ) {
			...
			msg += m.join(", ") + ".";
			msg += QCoreApplication::translate("KumIOArea","Garbage after input string.");
		}
	}
	return ok;
}
...
void validate2(QList<PeremType> L, QList<InputTerm> S, QList<InputTerm> &T, bool &R, int &G)
{
	...
	foreach ( l, L ) {
		...
		if ( l == boolean ) {
			t = S[i];
			bool ok;
			ok = t.text.toLower() == QCoreApplication::translate("KumIOArea","yes");
			ok = ok || t.text.toLower() == QCoreApplication::translate("KumIOArea","no");
			t.valid = ok;
			if ( ok )
				t.error = OK;
			else
