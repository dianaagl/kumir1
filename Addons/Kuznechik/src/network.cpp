...
if(!command.type!=HS_OK && !HSOK){
		...
		sendCmd(QString::fromUtf8("handshake,Кузнечик,1.0,1.0,eee\n"));
		return;
			};
...
bool KNPServer::OpenPort(QString addr,quint16 port)
{
   ...
   if(currentIsps.indexOf(QString::fromUtf8("Кузнечик,")+QString::number(port))<0)
	{
	AppendExtIspsToList(QString::fromUtf8("Кузнечик"),port);
	};
...
qWarning("Client Connected!!!!");
    ...
    sendMessage(clientConnection,"!Kuznechik 1.6 KNPServer\n");
onLine=true;
};
...
void KNPServer::reciveMessage()
{
	...
	sendMessage(tcpSocket,QString::fromUtf8("handshakeOK,Кузнечик,")+QString::number(VER)+"\n");
	Connected=true;
	...
	sendMessage(tcpSocket,QString::fromUtf8("Кузнечик 1.0\nhandshake,<CLIENT NAME>,<CLIENT VER>,<PROTOCOL VER>,<KEY>\nlist\n"));
...
tcpSocket->write(QString::fromUtf8("алг перекрасить\n").toUtf8());
...
if(command==QString::fromUtf8("вперед"))
	...
	if(KNPcommand.arguments.count()<1)return "ERROR,No argument.\n";
        int shag=KNPcommand.arguments[0].toInt();
	if(shag!=kuznecObj->Fstep)return QString::fromUtf8("ERROR,Плохой размер прыжка вперед:допустимо ")+QString::number(kuznecObj->Fstep)+".\n";
	if(!kuznecObj->canFwd())
	return QString::fromUtf8("ERROR,Прыжок вперед невозможен:выход за границы.\n");
          kuznecObj->MoveFwd();
	return "OK\n";	
	};
...
if(command==QString::fromUtf8("назад"))
	...
	if(KNPcommand.arguments.count()<1)return "ERROR,No argument.\n";
        int shag=KNPcommand.arguments[0].toInt();
	if(shag!=kuznecObj->Bstep)return QString::fromUtf8("ERROR,Плохой размер шага назад:допустимо ")+QString::number(kuznecObj->Bstep)+".\n";
	if(!kuznecObj->canBack())
	return QString::fromUtf8("ERROR,Прыжок назад невозможен:выход за границы.\n");
          kuznecObj->MoveBack();
	return "OK\n";
	};
if(command==QString::fromUtf8("перекрасить"))
	{
...
{
  kuznecObj->Kpult->show();
 kuznecObj->Kpult->raise();
 kuznecObj->Kpult->showNormal ();
};
void SignalCrosser::showWindow()
{
 kuznecObj->show();
 kuznecObj->raise();
 kuznecObj->showNormal ();
};