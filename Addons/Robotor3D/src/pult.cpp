...
void RoboPult::Up()
  {
  if(askStena->isChecked () ){emit update3D();emit hasUpWall(); askStena->setChecked(false);qWarning("Up Stena");;return;};
  if(askFree->isChecked () ){emit update3D();emit noUpWall(); askFree->setChecked(false);return;};
  emit goUp();
    emit update3D();
  };
void RoboPult::Down()
  {
  if(askStena->isChecked () ){emit update3D();emit hasDownWall(); askStena->setChecked(false);return;};
  if(askFree->isChecked () ){emit update3D();emit noDownWall(); askFree->setChecked(false);return;};
  emit goDown();
  emit update3D();
  };
void RoboPult::Left()
  {
  if(askStena->isChecked () ){emit update3D();emit hasLeftWall(); askStena->setChecked(false);return;};
  if(askFree->isChecked () ){emit update3D();emit noLeftWall(); askFree->setChecked(false);return;};
  emit goLeft();
  emit update3D();
  };
void RoboPult::Right()
  {
  if(askStena->isChecked () ){emit update3D();emit hasRightWall(); askStena->setChecked(false);return;};
  if(askFree->isChecked () ){emit update3D();emit noRightWall(); askFree->setChecked(false);return;};
  emit goRight();
  emit update3D();
  };
...
void RoboPult::CenterButton()
  {
  if(askStena->isChecked () ){emit update3D();emit Colored(); askStena->setChecked(false);return;};
  if(askFree->isChecked () ){emit update3D();emit Clean(); askFree->setChecked(false);return;};
  emit Color();
  emit update3D();
  };
