...
void OvenTimer::draw(QPainter *painter)
{
    ...
    QColor niceBlue(161, 161, 161);
    QColor sand(191, 191, 191);
    QColor sandLight(201, 201, 201);
...
NXTPult::NXTPult ( QWidget* parent, Qt::WFlags fl )
        : QWidget ( parent, fl ), Ui::TurtlePult()
{
    ...
    if(!buttRad->loadIcon(":/img/SoundNXT.png"))qWarning("Image not loaded!");
    TempB->hide();
    buttTemp=new MainButton(this);
    buttTemp->setGeometry(TempB->geometry());
    
    buttTemp->setText(" ");
    buttTemp->loadIcon(":/img/WallNXT.png");
...
void NXTPult::LinkOK(){qDebug()<<"LinkOK";link=true;greenLight->setLink(link);greenLight->repaint();ClearLog->setEnabled(true);};
...
void NXTPult::Left()
{
    ...
    turtleObj->DoRotate(-gradVal->value());
...
void NXTPult::TempS()
{
    if(!greenLight->link())return;
//    QString replay;
//    if(turtleObj->isWall())replay=QString(trUtf8("Да"));
//    else replay=QString(trUtf8("Нет"));
//    Logger->appendText(QString::fromUtf8("стена\n"),QString::fromUtf8("стена"),replay);
    turtleObj->CheckForWall();
};
void NXTPult::RadS()
{
    if(!greenLight->link())return;
    turtleObj->beep();
    Logger->appendText(QString::fromUtf8("звук\n"),QString::fromUtf8("звук"),"OK");
};

void NXTPult::resetTurtle()
{
    turtleObj->showNormal();
};
void NXTPult::newClient(QString message)
{
    label->setText(QString::fromUtf8("Подключился ")+message);
    toKumir->setEnabled(true);
}

void NXTPult::clientDisconnect()
{
...
void NXTPult::Connect(KNPServer* server)
{
    connect(server->SigCross,SIGNAL(do_move(int)),turtleObj,SLOT(DoMove(int)));
    connect(server->SigCross,SIGNAL(do_rotate(int)),turtleObj,SLOT(DoRotate(int)));
    connect(server->SigCross,SIGNAL(do_tailUp()),turtleObj,SLOT(TailUp()));
    connect(server->SigCross,SIGNAL(do_tailDown()),turtleObj,SLOT(TailDown()));



    connect(server,SIGNAL(newClient(QString)),this,SLOT(newClient(QString)));
    connect(server,SIGNAL(lockGui()),this,SLOT(noLink()));
    connect(server,SIGNAL(reset()),this,SLOT(resetTurtle()));
    connect(server,SIGNAL(unlockGui()),this,SLOT(LinkOK()));
    connect(server,SIGNAL(clientDisconnect()),this,SLOT(clientDisconnect()));
    Server=server;
};
void NXTPult::closeEvent ( QCloseEvent * event )
{
    ...
    int ret = QMessageBox::warning(this, QString::fromUtf8("Lego NXT"),
                                   QString::fromUtf8("Закрыть исполнитель Lego NXT?"),
