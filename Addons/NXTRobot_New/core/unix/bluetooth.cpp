...
void Bluetooth::connect(char *btaddr)
{
  int rv;
  ...
  if( status < 0 ) {
    ...
    throw Nxt_exception("connect","Bluetooth", UNABLE_TO_CONNECT);
    return;
  }
...
void Bluetooth::receive(unsigned char *buffer, unsigned int length)
{
  ...
  unsigned int i=0;
  int xr = 0;
  unsigned char *ucbyte = (unsigned char *) malloc(length*sizeof(unsigned char));
  byte = QByteArray(length,'\0');
  number_bytes = 0;
  buffer[0]='\0';
  //std::cerr<< " GOT SOCKET Status " << status << " READ " << doRead << std::endl;

  do {
      xr = ::read(nxtsock, ucbyte, length);
  } while (xr == -1 );
