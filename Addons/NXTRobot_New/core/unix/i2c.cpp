...
void I2c::wait_for_bytes(int num_bytes){
  ...
  do{
    ...
    //std::cerr << " WAITING FOR SENSOR BYTES " << wait_time << std::endl;
    _sleep(wait_time);

    if((timer.elapsed() > TIMEOUT)){
      std::cerr << " TIMEOUT DETECTED " << timer.elapsed() << std::endl;
      //throw Nxt_exception::Nxt_exception("wait_for_bytes","I2C", I2C_TIMEOUT);
    }
...
void I2c::update_sensor_status(){
  ...
  if(answer[4]){
    if(answer[4] != PENDING_COMMUNICATION){
