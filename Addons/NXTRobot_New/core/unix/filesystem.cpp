...
int Filesystem::get_first(string wild_card, Nxt_file *file){
  ...
  if(answer[4]){
     if((unsigned int) answer[4]!=  FILE_NOT_FOUND){
  ...
  try{
    ...
    if( e.error_code() != HANDLE_ALREADY_CLOSED){
      //throw Nxt_exception::Nxt_exception("get_first","Filesystem", 0x00FF & answer[4]);
    }
  }
  return 1;
...
unsigned short int Filesystem::read_command(Nxt_file &file, unsigned char *buffer, unsigned short int num_bytes){
  ...
  if(bytes_to_read != (num_bytes)){
    //throw Nxt_exception::Nxt_exception("read_command","Filesystem", UNDEFINED_FILE_ERROR);
  }
  i=0;
  while(i<num_bytes){
    buffer[i]=answer[8+i];
...
unsigned short int Filesystem::write_command(Nxt_file &file, unsigned char *buffer, unsigned short int num_bytes){
  ...
  if(bytes_written != (num_bytes)){
    //throw Nxt_exception::Nxt_exception("write_command","Filesystem", UNDEFINED_FILE_ERROR);
  }
