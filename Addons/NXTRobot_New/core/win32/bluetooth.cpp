...
void Bluetooth::connect(unsigned int comport){
  ...
  ***************************************************************************/
  ...
  temp << comport;
  QString port = "\\\\.\\COM" + QString::fromLocal8Bit(temp.str().c_str());
  handle = CreateFile(port.toStdWString().c_str(), GENERIC_READ | GENERIC_WRITE, 0,0,OPEN_EXISTING,0,0);
  if (handle == INVALID_HANDLE_VALUE){
...
/*
unsigned int Bluetooth::get_port(){
  return this->comport;
}
*/
