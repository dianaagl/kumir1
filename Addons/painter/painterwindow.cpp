    ...
    ui(new Ui::PainterWindow)
{
    ...
    m_zoomLabel->setText(QString::number(int(ui->view->zoom()*100))+"%");
...
void PainterWindow::handleCursorMoved(int x, int y, const QColor &color)
{
    const QString colorRgb = QString("rgb(%1,%2,%3)")
                             ...
                             .arg(color.green())
                             .arg(color.blue());
    const QString colorHumanName = PainterTools::CSS_RGB_toString(colorRgb);
    QString colorName;
    if (e_showColorMode=="RGB") {
    ...
    if (x==-1 || y==-1) {
        ...
        ui->statusbar->showMessage(QString::fromUtf8("Координаты: X = %1, Y = %2; Цвет: %3").arg(x).arg(y).arg(colorName));
    }
}
...
void PainterWindow::newImage()
{
    if (m_newImageDialog->exec()==QDialog::Accepted) {
        ...
        if (m_newImageDialog->isTemplate()) {
            setWindowTitle(QString::fromUtf8("Рисователь [%1]").arg(m_newImageDialog->templateName()));
        }
        else {
            setWindowTitle(QString::fromUtf8("Рисователь [новый лист]"));
        }
    }
}
...
void PainterWindow::loadImage()
{
    ...
    if (!fn.isEmpty() && QFile::exists(fn)) {
        ...
        setWindowTitle(QString::fromUtf8("Рисователь [%1]").arg(QFileInfo(fn).fileName()));
    }
}
...
void PainterWindow::saveImageAs()
{
    ...
    if (!fileName.isEmpty()) {
        ...
        if (s_templateName.isEmpty()) {
            setWindowTitle(QString::fromUtf8("%1 - Рисователь").arg(QFileInfo(fileName).fileName()));
        }
        else {
            setWindowTitle(QString::fromUtf8("%2 - Рисователь [%1]").arg(QFileInfo(s_templateName).fileName()).arg(QFileInfo(fileName).fileName()));
        }
