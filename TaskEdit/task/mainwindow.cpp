...
void MainWindowTask::setup()
{
	...
	ui->ispSel->addItem(trUtf8("Вертун"));
...
void MainWindowTask::saveCourseFile()
{
	...
	QMapIterator<int, int> i(changes);
	while (i.hasNext()) {
		i.next();
...
{
	if (newDialog->exec()) {
		QString fileName = QFileDialog::getSaveFileName(this, trUtf8("Сохранить курс..."), curDir, "*.kurs.xml");
...
void MainWindowTask::remField()
{
	...
	course->removeEnv(course->ispNodeByName(curTaskIdx, isp), ui->fieldsList->currentItem()->text());
	refreshIspsNEnv();
}
QString MainWindowTask::getFileTypes()
{
	...
	if (ui->ispList->currentItem()->text() == trUtf8("Вертун")) {
		return "(*.fil)";
	}
	return "(*.*)";
}
